import { useAuthorEarnings, useAuthorPayouts, useRequestPayout, useCancelPayout } from '@/hooks/useAuthor';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { DollarSign, Wallet, History, ArrowUpCircle, XCircle } from 'lucide-react';
...
export default function AuthorEarningsPage() {
  ...
  const { data: payouts, isLoading: payoutsLoading } = useAuthorPayouts();
  ...
  const handleRequestPayout = () => {
    ...
    requestPayout.mutate({
      amount,
      method: payoutMethod as 'bank_transfer' | 'paypal' | 'crypto',
      accountDetails: {}, // User would fill this in a real form
    }, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('pending_balance')}</CardTitle>
